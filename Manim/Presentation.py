...
"""This Number Only Works With 1080p 60fps."""
class LiveSmoothSpiral(Scene):
    def construct(self):
        # Central mass
        center = Circle(radius=0.25, color=RED).move_to(ORIGIN)
        self.add(center)

        # Initial state
        pos = np.array([-5.0, 3.8, 0.0])
        vel = np.array([2.5, 0.0, 0.0])  # Lowered for tighter spiral

        body = Dot(point=pos, radius=0.1, color=BLUE)
        path_points = [pos.copy()]
        self.add(body)

        # Gravity constant
        G = 18.0  # Increased to strengthen inward pull

        # Custom time scaling to compensate for high frame rate
        TIME_SCALE = 3.5

        def update_body(mob, dt):
            nonlocal pos, vel, path_points

            scaled_dt = TIME_SCALE * dt
            ...
            acc = G * r_vec / r**3
            vel += acc * scaled_dt
            vel *= 0.9935  # More subtle damping
            pos += vel * scaled_dt
            mob.move_to(pos)

            print(f"r = {r:.2f}, speed = {np.linalg.norm(vel):.2f}")

            # Only add point if it moved enough
            if np.linalg.norm(pos - path_points[-1]) > 0.01:
                path_points.append(pos.copy())


        body.add_updater(update_body)

        # Live-updating smooth path
        path = always_redraw(lambda: VMobject(stroke_color=WHITE, stroke_width=2)
                             .set_points_smoothly(path_points))
